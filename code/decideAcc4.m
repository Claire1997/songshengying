...
boundaryPoints_right(1,:) = boundaryPoints(:,1)- vehicle_array(i,1);
   ...
   if distance_foward_max - criticle_limite_intervehicle - 25 < 0 && distance_foward_max - criticle_limite_intervehicle > 5
        foward_flag = -1;
        Speed_future_v(2) = sqrt(2 * acc_max * distance_foward_max + Speed_foward^2 - 10);
        acc_v(2) = Speed_future_v(2) - Speed_v(2);
   elseif Speed_v(2)^2 -Speed_foward^2< (distance_foward_max -2*criticle_limite_intervehicle) * 2 * acc_brake/2
  ...
  if turn_flag == 0 && foward_flag == 0
      ...
      else acc_v(2) = -abs((Speed_foward^2 - Speed_v(2)^2)/2/(distance_foward_max - criticle_limite_intervehicle - 5));
      end
