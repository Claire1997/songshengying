function[acc_road acc_will] = decideAcc(i)
...
distance_boundary_right = abs(vehicle_array(i,1)+ vehicle_array(i,3)*cos(angle+pi/2) - boundaryPoints(floor(vehicle_array(i,2))+1,1));
distance_boundary_left = abs(vehicle_array(i,1)+vehicle_array(i,3)*cos(angle+pi/2) - boundaryPoints(floor(vehicle_array(i,2))+1,2));
critical =3;
