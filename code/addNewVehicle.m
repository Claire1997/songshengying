...
for i = 1:D
    if toll_barrier_state_line(i) == 1
        count = count + 1;
        vehicle(count,:) = [i * 4 - 2, 0, initial_speed, toll_barrier_config(3,i), 1,0,0];
    ...
    elseif toll_barrier_state_line(i) == 2
        count = count + 1;
        vehicle(count,:) = [i * 4 - 2, 0, initial_speed, toll_barrier_config(3,i), 2, 0, 0];
    elseif toll_barrier_state_line(i) == 3
        count = count + 1;
        vehicle(count,:) = [i * 4 - 2, 0, initial_speed, toll_barrier_config(3,i), 3, 0, 0];
    end
end
if count > 0
