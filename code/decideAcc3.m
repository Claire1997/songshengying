...
acc_max = 2;
acc_brake = 8;
Speed_v = [vehicle_array(i,3);vehicle_array(i,3)];
...
for j = 1: vehicle_number
    if i ~= j && vehicle_array(j,5) > 0  
        Speed_j = [vehicle_array(j,3);vehicle_array(i,3)];
...
boundaryPoints_right(1,:) = boundaryPoints(:,1)- vehicle_array(i,1);
   ...
   index_left = intersect(find(boundaryPoints_left(1,isPositiveLeft) > - width_veh(vehicle_array(i,5))/2), find(boundaryPoints_left(1,isPositiveLeft) <  width_veh(vehicle_array(i,5))/2 ));
   index_right = intersect(find(boundaryPoints_right(1,isPositiveRight) > - width_veh(vehicle_array(i,5))/2), find(boundaryPoints_right(1,isPositiveRight) <  width_veh(vehicle_array(i,5))/2 ));
  ...
  % if distance_foward_max - criticle_limite_intervehicle - 10 < 0 && distance_foward_max - criticle_limite_intervehicle > 5
  %     foward_flag = -1;
   
    if Speed_v(2)^2 -Speed_foward^2< (distance_foward_max - criticle_limite_intervehicle) * 2 * acc_brake/2
  ...
  if(foward_flag == 0 || foward_flag == -1)
     ...
     x_boundary_cross_left = interp1(boundaryPoints_left(:,1), boundaryPoints_left(:,2),delta_y_max,'spline');
     x_boundary_cross_right = interp1(boundaryPoints_left(:,1), boundaryPoints_left(:,2),delta_y_max,'spline');
     ...
     if alpha_min_l ~= 10 || alpha_min_r ~= 10||count ==0
         ...
         acc_v(1) = 0;
          if Speed_j_v(2) == 0
             acc_v(1) = 2*tan(alpha_min) * (Speed_j_v(2)+1/2*acc_max);
            
             acc_v(2) = 1/2 * acc_max;
          else
             acc_v(1) = 2*tan(alpha_min) * Speed_j_v(2)+vehicle_array(i,1); 
          end
  ...
  if turn_flag == 0 && foward_flag == 0
      ...
      else acc_v(2) = -abs((Speed_foward^2 - Speed_v(2)^2)/2/(distance_foward_max - criticle_limite_intervehicle));
      end
      acc_v = acc_v * 2;
